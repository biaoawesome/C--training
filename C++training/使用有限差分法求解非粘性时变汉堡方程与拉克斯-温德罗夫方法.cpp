    ...
    void solve() {
        ...
        for (int n = 0; n < Nt - 1; ++n) {
            for (int i = 1; i < Nx - 1; ++i) {
                double flux_right = u_current[i + 1] * u_current[i + 1];
                double flux_left = u_current[i - 1] * u_current[i - 1];
                double diffusion = u_current[i + 1] - 2 * u_current[i] + u_current[i - 1];

                u_new[i] = u_current[i]
                    - (dt / (2 * dx)) * (flux_right - flux_left)
                    + (dt * dt / (2 * dx * dx)) * diffusion;
            }
